import {EventReceiver, midiToFrequency, RMS} from "../modules/neutrons.js";
...
class KSString {
    ...
    process(outputs, fromIndex, toIndex) {
        const ch0 = outputs[0];
        const ch1 = outputs[1];
        const coeff = this.stroking ? 0.498 : 0.4;
        ...
        for (let i = fromIndex; i < toIndex; i++) {
            ...
            if (this.bodyEnabled) {
                const sin = Math.sin(this.phase * 2.0 * Math.PI) * energy * energy * 0.3;
                ch0[i] += sin;
                ch1[i] += sin;
            ...
            ch1[i] += this.line[(this.index + phaseShift) % this.length] * 0.2;
    ...
    constructor(options) {
        ...
        this.port.onmessage = event => {
            ...
            switch (data.action) {
                ...
                case "play": {
                    this.receiver.enqueue(new PlayEvent(args.time, args.index, args.offset + this.capo));
                    break;
                }
                case "damp": {
